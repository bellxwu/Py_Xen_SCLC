...
@author: bellwu
'''
# %% Ordered Violin plot function
def OrderedViolinPlots(adata, GeneList, ColumnOfInterest, AddEmpty=True):
    '''
    Generate ordered violin plots of multiple gene inputs at once using scanpy violin
    plot wrapper
    '''
    import scanpy as sc
    import warnings

    # ensure GeneList is within the rownames of the anndara
    if not any(adata.var_names.isin(GeneList)):
        warnings.warn(
            "Listed genes not found in anndata",
            TypeError
        )

    # plotting function of one gene
    ...
    # for loop to output plot of gene hits as dictionary key-value
    plots = {}
